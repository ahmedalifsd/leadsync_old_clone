...
$(document).ready(function(){
    ...
    /* ═══════════════════════════════════════════════════════════
   card-animations.js  —  place before </body>
   Particles now show on ALL .card-animated cards (dark + white)
   ═══════════════════════════════════════════════════════════ */

document.addEventListener('DOMContentLoaded', function () {

  /* ── inject animation layers into .card-animated cards only ── */
  document.querySelectorAll('.stat-card.card-animated').forEach(function (card, i) {
    ...
    /* shimmer — staggered per card */
    var shimmer = document.createElement('div');
    shimmer.className = 'ca-shimmer';
    shimmer.style.animationDelay = (i * 0.7) + 's';
    card.prepend(shimmer);

    /* corner glow */
    var glow = document.createElement('div');
    glow.className = 'ca-corner-glow';
    glow.style.animationDelay = (i * 0.4) + 's';
    card.prepend(glow);

    /* wave */
    var wave = document.createElement('div');
    wave.className = 'ca-wave';
    wave.style.animationDelay = (i * 0.5) + 's';
    card.prepend(wave);

    /* 3 particles — injected on ALL cards (dark + white) */
    var pDelays = [0, 1.2, 2.5];
    for (var p = 0; p < 3; p++) {
      var particle = document.createElement('div');
      particle.className = 'ca-particle';
      particle.style.animationDelay = pDelays[p] + 's';
      card.prepend(particle);
    }
  });

  /* ── chart hover-replay — ALL .stat-card elements ── */
  document.querySelectorAll('.stat-card').forEach(function (card, i) {

    triggerLoad(card, 120 + i * 65);

    card.addEventListener('mouseenter', function () {
      card.classList.remove('card-loaded', 'restart');
      card.classList.add('card-hovered');
      void card.offsetWidth;
      setTimeout(function () { card.classList.add('restart'); }, 30);
    });

    card.addEventListener('mouseleave', function () {
      card.classList.add('card-hovered', 'restart'); /* fixed: remove not add */
    });
  });

  function triggerLoad(card, delay) {
    card.classList.add('card-loaded', 'card-hovered', 'restart');
    void card.offsetWidth;
    setTimeout(function () { card.classList.add('card-loaded'); }, delay || 10);
  }
